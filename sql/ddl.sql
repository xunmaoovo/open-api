-- 创建库
create database if not exists xunapi;
...
-- 接口信息
create table if not exists xunapi.`interface_info`
(
...
insert into xunapi.`interface_info` (`name`, `description`, `url`, `requestHeader`, `responseHeader`, `status`,
                                     `method`, `userId`)
...
create table if not exists xunapi.`user_interface_info`
(
    `id`             bigint                             not null auto_increment comment '主键' primary key,
